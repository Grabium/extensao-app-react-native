...
1.  **Importação:** Importamos `NetInfo`.
2.  **`NetInfo.fetch()`:** Retorna uma Promise e resolve com um objeto literal com informações sobre o estado da conexção coma a rede.
3.  **`state.type`:** Tipo de conexão: `wifi`, `cellular`, `unknown`.
4.  **`state.isConnected`:** Mostra se está conectado: true ou false.
